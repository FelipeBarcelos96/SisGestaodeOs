...
public class OrdemHandler extends Handler {

	public OrdemHandler(ObjectMapper objectMapper, GlobalExceptionHandler exceptionHandler) {
		...
		if ("GET".equals(exchange.getRequestMethod())) {
			...
			try {
				if(Integer.parseInt(id) >= 0) {
					Ordem ordem = OrdemDao.getInstance().get(Integer.parseInt(id));
					resp = new Gson().toJson(ordem);
				}else {
					ArrayList<Ordem> ordems = OrdemDao.getInstance().getAll();
					
						resp = new Gson().toJson(ordems);
