...
public class EquipeHandler extends Handler {
	...
	public EquipeHandler(ObjectMapper objectMapper, GlobalExceptionHandler exceptionHandler) {
		...
		if ("GET".equals(exchange.getRequestMethod())) {
			...
			try {
				if(Integer.parseInt(id) >= 0) {
					Equipe equipe = EquipeDao.getInstance().get(Integer.parseInt(id));
					resp = new Gson().toJson(equipe);
				}else {
					ArrayList<Equipe> equipes = EquipeDao.getInstance().getAll();
					
						resp = new Gson().toJson(equipes);
