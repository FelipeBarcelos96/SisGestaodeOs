...
public class UsuarioHandler extends Handler {
	...
	public UsuarioHandler(ObjectMapper objectMapper, GlobalExceptionHandler exceptionHandler) {
		...
		if ("GET".equals(exchange.getRequestMethod())) {
			...
			try {
				if(Integer.parseInt(id) >= 0) {
					Usuario user = UsuarioDao.getInstance().get(Integer.parseInt(id));
					resp = new Gson().toJson(user);
				}else {
					ArrayList<Usuario> usuarios = UsuarioDao.getInstance().getAll();
					
						resp = new Gson().toJson(usuarios);
			...
			try {
				if(Integer.parseInt(id) > 0) {
					UsuarioDao.getInstance().delete(Integer.parseInt(id));					
				}
			}catch(Exception e) {
				exchange.sendResponseHeaders(StatusCode.BAD_REQUEST.getCode(), -1);
				System.out.println("Error! Alert: ");
