...
public class RequisitoHandler extends Handler {

	public RequisitoHandler(ObjectMapper objectMapper, GlobalExceptionHandler exceptionHandler) {
		...
		if ("GET".equals(exchange.getRequestMethod())) {
			...
			try {
				if(Integer.parseInt(id) >= 0) {
					Requisito requisito = RequisitoDao.getInstance().get(Integer.parseInt(id));
					resp = new Gson().toJson(requisito);
				}else {
			...
			try {
				if(Integer.parseInt(id) > 0) {
					RequisitoDao.getInstance().delete(Integer.parseInt(id));					
				}
			}catch(Exception e) {
				resp = e.getMessage();
